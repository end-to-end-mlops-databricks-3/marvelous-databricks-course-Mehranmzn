...
from game_sales.utils import adjust_predictions


class GameSalesModelWrapper(mlflow.pyfunc.PythonModel):
    """Wrapper class for machine learning models to be used with MLflow.

    This class wraps a machine learning model for predicting game prices.
    """

    def __init__(self, model: object) -> None:
        """Initialize the GameSalesModelWrapper.
...
class CustomModel:
    """Custom model class for game sales prediction.
    ...
    def log_model(self, dataset_type: Literal["PandasDataset", "SparkDataset"] = "SparkDataset") -> None:
        ...
        with mlflow.start_run(tags=self.tags) as run:
            ...
            mlflow.pyfunc.log_model(
                python_model=GameSalesModelWrapper(self.pipeline),
                artifact_path="pyfunc-game-sales-model",
    ...
    def register_model(self) -> None:
        ...
        registered_model = mlflow.register_model(
            model_uri=f"runs:/{self.run_id}/pyfunc-game-sales-model",
            name=f"{self.catalog_name}.{self.schema_name}.game_sales_model_custom",
            tags=self.tags,
        )
    ...
    def load_latest_model_and_predict(self, input_data: pd.DataFrame) -> np.ndarray:
        ...
        model_uri = f"models:/{self.catalog_name}.{self.schema_name}.game_sales_model_custom@latest-model"
